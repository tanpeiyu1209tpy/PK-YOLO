...
class BboxLoss(nn.Module):
    ...
    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes, target_scores, target_scores_sum, fg_mask):
        ...
        # ---------------------------------------------
        device = loss_iou.device
        oiou = bbox_iou(pred_bboxes_pos, target_bboxes_pos, xywh=False).squeeze()
        d = torch.tensor(0.00, device=device)
        u = torch.tensor(0.95, device=device)
        focaleriou = ((oiou - d) / (u - d)).clamp(0, 1)  # default d=0.00,u=0.95
        focaleriou = torch.where(oiou <= u, focaleriou, torch.full_like(oiou, 1.0, device=device))
        lfocaler = (oiou - focaleriou).mean()
        loss_iou += lfocaler
        # print("----->",loss_iou)
        # ---------------------------------------------
